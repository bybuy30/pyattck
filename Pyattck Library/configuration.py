...
class Options:
    ...
    def _download_url_data(self, url):
        ...
        if response.status_code != 200:
            warnings.warn(
                message=f"Unable to download data from {url}. HTTP {response.status_code}")
            return {}
        # Validate content type and JSON body
        content_type = response.headers.get("Content-Type", "").lower()
        if "json" not in content_type and not response.text.strip().startswith("{"):
            warnings.warn(message=f"Response from {url} is not JSON. Content-Type: {content_type}")
            return {}
        try:
            return response.json()
        except Exception:
            # Fallback to raw load to catch partial/valid JSON in body
            try:
                return json.loads(response.text)
            except Exception:
                warnings.warn(message=f"Received invalid JSON from {url}. Returning empty result.")
                return {}
